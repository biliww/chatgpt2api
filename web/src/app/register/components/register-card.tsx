...
export function RegisterCard() {
  const config = useRegisterStore((state) => state.registerConfig);
  const isLoading = useRegisterStore((state) => state.isLoadingRegister);
  const isSaving = useRegisterStore((state) => state.isSavingRegister);
  const setProxy = useRegisterStore((state) => state.setRegisterProxy);
  const setTotal = useRegisterStore((state) => state.setRegisterTotal);
  const setThreads = useRegisterStore((state) => state.setRegisterThreads);
  const setMode = useRegisterStore((state) => state.setRegisterMode);
  const setTargetQuota = useRegisterStore((state) => state.setRegisterTargetQuota);
  const setTargetAvailable = useRegisterStore((state) => state.setRegisterTargetAvailable);
  const setCheckInterval = useRegisterStore((state) => state.setRegisterCheckInterval);
  const setMailField = useRegisterStore((state) => state.setRegisterMailField);
  const addProvider = useRegisterStore((state) => state.addRegisterProvider);
  const updateProvider = useRegisterStore((state) => state.updateRegisterProvider);
  const deleteProvider = useRegisterStore((state) => state.deleteRegisterProvider);
  const save = useRegisterStore((state) => state.saveRegister);
  const toggle = useRegisterStore((state) => state.toggleRegister);
  const reset = useRegisterStore((state) => state.resetRegister);
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          {config.mode === "total" && (
            <div className="space-y-2">
              ...
              <Input value={String(config.total)} onChange={(event) => setTotal(event.target.value)} className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
            </div>
          )}
          <div className="space-y-2">
          ...
          {config.mode === "quota" && (
            <div className="space-y-2">
              ...
              <Input value={String(config.target_quota || "")} onChange={(event) => setTargetQuota(event.target.value)} className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
            </div>
          )}
          {config.mode === "available" && (
            <div className="space-y-2">
              <label className="text-sm text-stone-700">目标可用账号</label>
              <Input value={String(config.target_available || "")} onChange={(event) => setTargetAvailable(event.target.value)} className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
            </div>
          )}
          {config.mode !== "total" && (
            <div className="space-y-2">
              <label className="text-sm text-stone-700">检查间隔（秒）</label>
              <Input value={String(config.check_interval || "")} onChange={(event) => setCheckInterval(event.target.value)} className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
            </div>
          )}
        </div>
              ...
              return (
                <div key={index} className="space-y-3 border-t border-stone-200 pt-3 first:border-t-0 first:pt-0">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    {["cloudmail_gen", "cloudflare_temp_email", "moemail", "inbucket", "yyds_mail", "ddg_mail"].includes(type) ? (
                      <>
                        <div className="space-y-2">
                        ...
                        {["cloudflare_temp_email", "ddg_mail"].includes(type) && (
                          <div className="space-y-2">
                    ...
                    {["tempmail_lol", "moemail", "duckmail", "gptmail", "yyds_mail"].includes(type) && (
                      <div className="space-y-2">
                    ...
                    {["duckmail", "gptmail"].includes(type) && (
                      <div className="space-y-2">
                  ...
                  {["cloudmail_gen", "tempmail_lol", "cloudflare_temp_email", "moemail", "inbucket", "yyds_mail", "ddg_mail"].includes(type) && (
                    <div className="space-y-2">
                      <label className="text-sm text-stone-700">{type === "cloudmail_gen" ? "邮箱域名" : type === "inbucket" ? "基础域名列表" : "Domain"}</label>
                      <Textarea value={domains} onChange={(event) => updateProvider(index, { domain: event.target.value.split(/[\n,]/).map((item) => item.trim()) })} placeholder={type === "inbucket" ? "每行一个基础域名，系统会自动生成随机子域名" : "每行一个域名，留空则使用服务默认域名"} className="min-h-20 rounded-xl border-stone-200 bg-white font-mono text-xs" disabled={config.enabled} />
                    </div>
                  )}
                  {type === "cloudmail_gen" && (
                    <div className="space-y-2">
